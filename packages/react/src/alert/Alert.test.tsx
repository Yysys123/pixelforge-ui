...
describe('Alert', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders with custom style', () => {
      const { container } = render(
        <Alert style={{ color: 'red' }}>Test</Alert>
      );
      const alert = container.firstChild as HTMLElement;
  ...
  describe('Auto-hide functionality', () => {
    it('auto-hides after specified duration', () => {
      ...
      render(
        <Alert autoHideDuration={3000} onDismiss={handleDismiss}>
          Test
        </Alert>
      );
    ...
    it('does not auto-hide when duration is 0', () => {
      ...
      render(
        <Alert autoHideDuration={0} onDismiss={handleDismiss}>
          Test
        </Alert>
      );
  ...
  describe('Toast positioning', () => {
    ...
    it.each([
      ...
      'bottom-right',
      'bottom-center',
    ] as const)('applies %s position class when toast', position => {
      const { container } = render(
        <Alert toast position={position}>
          Test
        </Alert>
      );
      const alert = container.firstChild as HTMLElement;
  ...
  describe('Custom styling', () => {
    ...
    it('combines rotation with other transforms', () => {
      const { container } = render(
        <Alert rotation={-5} style={{ transform: 'scale(1.1)' }}>
          Test
        </Alert>
      );
      const alert = container.firstChild as HTMLElement;
  ...
  describe('Complex examples', () => {
    ...
    it('renders notification-style alert', () => {
      ...
      expect(
        screen.getByText('You have received a new message from John.')
      ).toBeInTheDocument();
    ...
    it('renders minimal alert', () => {
      render(
        <Alert variant="warning" showIcon={false} showDecorations={false}>
  ...
  describe('HTML Attributes', () => {
    it('forwards additional props', () => {
      render(
        <Alert data-testid="custom-alert" aria-describedby="help-text">
          Test
        </Alert>
      );
  ...
  describe('Accessibility', () => {
    ...
    it('supports custom role for different alert types', () => {
    ...
    it('maintains focus management when dismissible', async () => {
      ...
      render(
        <div>
          ...
          <Alert dismissible>Test alert</Alert>
    ...
    it('provides keyboard navigation for dismiss button', async () => {
      const user = userEvent.setup({ advanceTimers: jest.advanceTimersByTime });
    ...
    it('supports high contrast mode', () => {
      ...
      render(<Alert variant="warning">High contrast alert</Alert>);
      expect(screen.getByRole('alert')).toBeInTheDocument();
    });
    ...
    it('respects reduced motion preferences', () => {
      ...
      render(<Alert dismissible>Reduced motion alert</Alert>);
      expect(screen.getByRole('alert')).toBeInTheDocument();
    });

    it('properly associates title with content for screen readers', () => {
      render(
        <Alert title="Error Occurred" variant="danger">
    ...
    it('maintains semantic structure for complex alerts', () => {
      const actions = (
        <div>
          <button>Retry</button>
    ...
    it('handles focus when alert auto-dismisses', async () => {
      ...
      render(
        <div>
          ...
          <Alert autoHideDuration={100} onDismiss={onDismiss}>
            Auto-dismiss alert
          </Alert>
        </div>
      );
    ...
    it('announces alert content changes to screen readers', async () => {
      const { rerender } = render(<Alert>Initial message</Alert>);
      expect(screen.getByText('Initial message')).toBeInTheDocument();

      rerender(<Alert>Updated message</Alert>);
